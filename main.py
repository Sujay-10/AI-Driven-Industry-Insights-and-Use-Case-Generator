...
def main():
    ...
    if st.button("Submit"):
        if industry:
            ...
            with open("use_cases.txt", "w") as f:
                f.write(f"Proposed Use Cases:\n\n{use_cases.get('text', '') if isinstance(use_cases, dict) else str(use_cases)}\n")
            
            with open("resources.txt", "w") as f:
                f.write(f"Resource Links:\n\n{resources.get('text', '') if isinstance(resources, dict) else str(resources)}\n")
    ...
    if st.session_state.use_cases and st.session_state.resources:
        ...
        if output_option == "Use Cases":
            ...
            st.markdown(st.session_state.use_cases.get('text', '') if isinstance(st.session_state.use_cases, dict) else str(st.session_state.use_cases))
        else:
            st.subheader("Resource Links")
            st.markdown(st.session_state.resources.get('text', '') if isinstance(st.session_state.resources, dict) else str(st.session_state.resources))
